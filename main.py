...
def start(update, context):
    ...
    update.message.reply_text(
        ...
        'будут замки, рыцари и драконы. В течение всего повествования будет несколько моментов, '
...
def gorod(update, context):
    user = update.message.from_user
    logger.info(f'{user.first_name} дошел до ЧЕКПОИНТА.')
    update.message.reply_text('Вы с лёгкостью прошли в город и стали в нём жить.')
    logger.info(f'{user.first_name} завершил историю.')
    update.message.reply_text('ПРОДОЛЖЕНИЕ СЛЕДУЕТ...')
